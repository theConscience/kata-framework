...
var CONFIG_PAGE_NEWS_REPOST = (function(settings, utils, baseConfig, subConfig, $) {
  ...
  function initCustomTimepickers(config) {
    // custom-timepickers components initialization scripts

    // configuration //
    var customTimepickers = document.querySelectorAll('.' + config.TITLE); // NodeList

    // multiple elements callbacks with adding browser event listeners //
    ...
    function customTimepickerCallback(index, node) {
      ...
      // node.dataset.deaultOffset = parseFloat(_getBackdropTransformREM(backdrop), 10).toPrecision(4).replace(/0*$/gi, '') + 'rem';  // первые скобки - fix Для IE
      ...
      input.value = activeBackdropItem
        ? activeBackdropItem.getAttribute(config.DATA.BACKDROP_ITEM_VALUE)
    ...
    */
    ...
    function inputBlurHandler(evt) {
      ...
      var enteredValue = 0;
      var symbolsCount = evt.target.getAttribute('pattern').match(/\[0-9\]\{(\d*)\}/, 'gi');
      if (symbolsCount) {
        var symbolsCountValue = parseFloat(symbolsCount[1], 10); // get symbols count from html attribute
        if (LOGGER.DETAILED) console.log('symbolsCountValue is:', symbolsCountValue);
        enteredValue = evt.target.value = evt.target.value.replace(/^0+/i, '').slice(0, symbolsCountValue) || '0';
      } else {
    ...
    function changeCustomTimepickerValue(customTimepicker, direction) {
      if (LOGGER.DETAILED)
        console.log('customTimepicker:', customTimepicker, ' is activated, gonna switch to', direction, 'values');
      ...
      if (LOGGER.DETAILED)
        console.log('customTimepicker.dataset.activeBackdropItem:', customTimepicker.dataset.activeBackdropItem);
      var prevActiveItemIndex = Math.abs(parseFloat(customTimepicker.dataset.activeBackdropItem, 10));
      ...
      var backdropCssStartTransformPropertyREM = _getBackdropTransformREM(backdrop);

      var newBackdropTransformValueREM = 0;
      if (direction === 'prev') {
        if (rollBackward) {
          newBackdropTransformValueREM =
            -parseFloat(getComputedStyle(backdrop).width, 10) / htmlRootFontSize + activeBackdropItemCssWidthREM; //  + backdropDefaultOffsetREM
        } else {
    ...
    function setCustomTimepickerValue(customTimepicker, value) {
      if (LOGGER.DETAILED)
        console.log('customTimepicker:', customTimepicker, ' is activated, gonna switch to value', value);
      customTimepicker.dataset.isBlocked = true;

      var backdrop = customTimepicker.querySelector('.' + config.ELEMENTS.BACKDROP);
      var prevActiveBackdropItem = backdrop.querySelector('.' + config.MODIFIERS.BACKDROP_ITEM_ACTIVE);
      if (LOGGER.DETAILED) console.log('prevActiveBackdropItem:', prevActiveBackdropItem);
      if (prevActiveBackdropItem) prevActiveBackdropItem.classList.remove(config.MODIFIERS.BACKDROP_ITEM_ACTIVE);
      var activeBackdropItem = backdrop.querySelector(
        '.' + config.ELEMENTS.BACKDROP_ITEM + '[' + config.DATA.BACKDROP_ITEM_VALUE + '="' + value + '"]'
      ); // NodeList
      if (LOGGER.DETAILED) console.log('activeBackdropItem:', activeBackdropItem);
      ...
      var backdropItems = backdrop.querySelectorAll('.' + config.ELEMENTS.BACKDROP_ITEM); // NodeList
      if (LOGGER.DETAILED) console.log('valu:', value);
      var firstItemDataValue = parseFloat(backdropItems[0].dataset.value, 10);
      if (LOGGER.DETAILED) console.log('firstItemDataValue:', firstItemDataValue);
      if (LOGGER.DETAILED) console.log('value - firstItemDataValue:', value - firstItemDataValue);

      customTimepicker.dataset.activeBackdropItem = value - firstItemDataValue;
      ...
      var newBackdropTransformValueREM =
        (backdropLeftOffset - activeBackdropItemLeftOffset + backdropPaddingLeft) / htmlRootFontSize; // + backdropDefaultOffsetREM;
    ...
    function _getBackdropTransformREM(backdrop) {
      ...
      if (backdropCssTransformMatrix && backdropCssTransformMatrix[1]) {
        ...
        var htmlRootFontSize = parseFloat(getComputedStyle(document.documentElement).fontSize, 10);
        if (LOGGER.DETAILED) console.log('htmlRootFontSize:', htmlRootFontSize);
        var backdropCssStartTransformPropertyREM = parseFloat(
          (backdropCssStartTransformProperty / htmlRootFontSize).toPrecision(4).replace(/0*$/gi, ''),
          10
        );
